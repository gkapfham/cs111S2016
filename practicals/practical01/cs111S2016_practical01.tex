...
\MYTITLE{Practical 1\\Assigned: Friday, January 22, 2016\\Due: Friday, January 29, 2016 at the start of class\\``Checkmark'' grade}
...
\begin{enumerate}
  ...
  \item Now, you need to test to see if you can authenticate with the Bitbucket servers.  First, show the course
    ...
    mkdir cs111S2015}'' you could make a {\tt cs111S2015/} directory that will contain the Git repository that the
    instructor will always use to share files with you.  Once you have done so, please type the following command 
    ``{\tt git clone git@bitbucket.org:gkapfham/cs111s2015-share.git}''.
  ...
  \item Using your terminal window, you should browse the files that are in this Git repository.  In particular, please
    look in the {\tt cs111s2015-share/practicals/practical01/} directory and use {\tt gvim} to study the Java program that
    ...
    Congratulations on completing these steps to access the {\tt cs111S2016-share} repository that we will use
...
cs111S2015/} directory that you previously created.  First, make a new directory called {\tt cs111S2015-<your user
...
them with your neighbor, a teaching assistant, a departmental tutor, or the course instructor if you are confused.
    ...
    \begin{verbatim}
        cd cs111S2015
        mkdir cs111S2015-<your user name>
        cd cs111S2015-<your user name>
    \end{verbatim}
    \vspace*{-.2in}
...
you completed each of these steps inside of the parent directory called {\tt cs111S2015-<your user name>}; if you are
...
window, which we will call the ``source'' window, you should navigate to the {\tt cs111s2015-share} directory by
clicking on the correct folder icons.  Now, please find the {\tt Kinetic.java} file that is stored inside of the {\tt
  cs111s2015-share} repository in the ``source'' window. In the second window, which we will call the ``destination''
window, you should follow the same process to first navigate to the {\tt cs111S2015-<your user name>} directory,
...
``source'' browser to the ``destination'' browser, ensuring that the file is transferred from the {\tt cs111s2015-share}
repository to the {\tt practical01/} directory of the {\tt cs111S2015-<your user name>} repository.  Once the files are
